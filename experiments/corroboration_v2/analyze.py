...
def main() -> None:
    ...
    upgraded = sum(
        1 for r in results if r["corroborated"] and r.get("reg_grade") != r.get("upgraded_grade")
    )
    ...
    if fired / max(1, total) >= 0.95 and has_url / max(1, total) >= 0.9:
        ...
        print(
            f"   • {nc.get('passed', 0) if nc else 0}/{nc.get('total', 0) if nc else 0} negative controls pass"
        )
