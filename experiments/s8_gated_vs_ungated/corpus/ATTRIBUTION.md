...
- **531 chunk files** in `chunks/` are real PDF extractions from the official
  OpenStax and lightandmatter.com URLs, verified by SHA-256 checksums (recorded
  in `corpus/CHECKSUMS.txt`) and extracted via pdfplumber. Each chunk header
  includes the source PDF, license, and page range. Example header:
...
- Claims/spans are extracted from the chunks by a **deterministic heuristic
  segmenter + filter** (`extract.py`) — sentence-level spans, not LLM "atomic
  claims" (issue #94).
