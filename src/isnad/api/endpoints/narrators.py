...
async def register_narrator(body: dict, reg: RegistryDB = Depends(get_registry), _: str = Depends(require_admin)) -> dict:
    grade_map = {
        "reliable": NarratorGrade.RELIABLE, "acceptable": NarratorGrade.ACCEPTABLE,
        "weak": NarratorGrade.WEAK, "rejected": NarratorGrade.REJECTED,
    ...
    reg.flush()
    return {"narrator_id": body["narrator_id"], "domain": body.get("domain", "general"), "grade": grade.value}
...
async def submit_evidence(body: dict, reg: RegistryDB = Depends(get_registry), _: str = Depends(require_admin)) -> dict:
    try:
    ...
    new_grade = reg.registry.record_evidence(
        body["narrator_id"], body.get("domain", "general"),
        ev_type, ev_action, body.get("description", ""),
    )
    reg.flush()
    if old_grade is not None and new_grade != old_grade:
        _metrics_counters["bayesian_grade_changes_total"] += 1
    return {"narrator_id": body["narrator_id"], "domain": body.get("domain", "general"), "new_grade": new_grade.value}
