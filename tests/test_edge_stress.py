...
with get_session() as session:
    ...
    chain2 = Chain([ChainLinkSpec("src", 0, domain="physics"),
    ...
    try:
        ...
        check("Updated chain has 2 links", len(c2.links) == 2,
              f"got {len(c2.links)}")
    ...
    try:
        ...
        check("Re-store case-variant -> NO crash", True,
              f"links={len(c3.links)}")
    except Exception as e:
...
r1 = client.post("/v1/claims", json={
    "claim_text": "Newton second law", "domain": "physics",
...
check("API re-submit same text -> 200 (was crash)", r2.status_code == 200,
...
cg_incomplete = grade_chain(
    [NarratorGrade.RELIABLE], [TransformType.PASS_THROUGH],
    is_complete=False,
)
...
check("UNGRADED + RELIABLE -> HASAN (cap)", cg_unknown == ChainGrade.HASAN,
...
check("Grade is not empty (defaults to UNGRADED->HASAN cap)",
      r.json()["chain_grade"] != "", f"grade={r.json()['chain_grade']}")
...
for i in range(20):
    ...
    g = reg_osc.record_evidence("pingpong", "physics",
                                EvidenceType.POST_HOC_AUDIT, action,
                                f"event {i}")
    grades_seen.append(g)

final = reg_osc.get_grade("pingpong", "physics")
check("Ping-pong converges to WEAK (Beta(11,11) mean=0.5)",
      final == NarratorGrade.WEAK, f"got {final.value}")
...
for i in range(50):
    ...
    all_chains.append({
        "claim_text": claim_text, "chain_grade": chain_grade,
        "narrator_ids": narrator_ids, "source": f"page_{i}",
    })
...
result_50 = engine.evaluate(
    ...
    all_chains=all_chains, narrator_metadata={},
)
check("50 claims, 10 corroborating -> upgrade fires", result_50.upgraded,
...
g1 = reg_swap.record_evidence("swapper", "physics",
                              EvidenceType.POST_HOC_AUDIT, EvidenceAction.JARH)
check("Bayesian: 1 JARH from cold start -> REJECTED (Beta(1,2) mean=0.33)",
      g1 == NarratorGrade.REJECTED, f"got {g1.value}")
...
for _ in range(3):
    reg_swap.record_evidence("swapper2", "physics",
                             EvidenceType.POST_HOC_AUDIT, EvidenceAction.JARH)
g2 = reg_swap.get_grade("swapper2", "physics")
check("Threshold after swap: 3 JARH -> ACCEPTABLE",
...
check("Bayesian fresh instance: re-evaluates from evidence log",
      g3 == NarratorGrade.REJECTED, f"got {g3.value}")
...
for i in range(1000):
    ...
    reg_massive.record_evidence("heavy", "physics",
                                EvidenceType.EVAL_HARNESS, action, f"ev-{i}")
...
check("1000 evidence entries stored", evidence_count == 1000,
      f"got {evidence_count}")
# 700 TADIL + 300 JARH -> Beta(701,301) mean=0.6996 < 0.75 -> WEAK
check("700 TADIL + 300 JARH -> WEAK (Beta(701,301) mean=0.70 < 0.75)",
      final_grade == NarratorGrade.WEAK, f"got {final_grade.value}")
...
deep_links = [ChainLinkSpec(f"step-{i}", i, domain="physics",
...
check("99 RELIABLE + 1 WEAK -> DAIF (100 links deep)",
      cg_deep == ChainGrade.DAIF, f"got {cg_deep.value}")

with get_session() as session:
    stored = store_claim(session, "deep chain test", "test/deep", deep_chain)
    check("Deep chain stored", stored is not None)
    check("Deep chain links persisted", len(stored.links) == 100,
          f"got {len(stored.links)}")
    session.rollback()
...
check("Same narrator, physics=RELIABLE",
      reg_dom.get_grade("multi-model", "physics") == NarratorGrade.RELIABLE)
check("Same narrator, history=WEAK",
      reg_dom.get_grade("multi-model", "history") == NarratorGrade.WEAK)
check("Same narrator, chemistry=ACCEPTABLE",
      reg_dom.get_grade("multi-model", "chemistry") == NarratorGrade.ACCEPTABLE)
check("Same narrator, biology=UNGRADED",
...
check("Mixed-domain chain: weak link -> DAIF",
      cg_mixed == ChainGrade.DAIF, f"got {cg_mixed.value}")
...
check(f"200 claims: {success} OK, {fail} failed", fail == 0,
...
list_data = r.json()
check("Claims list shows all 200", list_data["total"] == 200,
...
result_mix = engine2.evaluate(
    ...
    all_chains=[
        {"claim_text": "the earth orbits the sun", "chain_grade": "hasan",
         "narrator_ids": ["gpt4"], "source": ""},
        {"claim_text": "the earth orbits the sun", "chain_grade": "hasan",
         "narrator_ids": ["gemini_pro"], "source": ""},
    ],
    narrator_metadata={
...
check("Different model families -> independent -> upgrade", result_mix.upgraded,
...
result_same_source = engine2.evaluate(
    ...
    all_chains=[
        {"claim_text": "water boils at 100 celsius", "chain_grade": "hasan",
         "narrator_ids": ["scraper_wiki_v2"], "source": ""},
        {"claim_text": "water boils at 100 celsius", "chain_grade": "hasan",
         "narrator_ids": ["scraper_britannica"], "source": ""},
    ],
    narrator_metadata={
...
check("Shared upstream source -> partial discount",
...
result_no_meta = engine2.evaluate(
    ...
    base_narrators=["a", "b"],
    all_chains=[
        {"claim_text": "speed of light is constant", "chain_grade": "hasan",
         "narrator_ids": ["c", "d"], "source": ""},
        {"claim_text": "speed of light is constant", "chain_grade": "hasan",
         "narrator_ids": ["e", "f"], "source": ""},
    ],
    narrator_metadata={},
)
check("No metadata -> assumed independent -> upgrade", result_no_meta.upgraded,
...
result_partial_meta = engine2.evaluate(
    ...
    all_chains=[
        {"claim_text": "newton discovered gravity", "chain_grade": "hasan",
         "narrator_ids": ["historian_2"], "source": ""},
        {"claim_text": "newton discovered gravity", "chain_grade": "hasan",
         "narrator_ids": ["historian_3"], "source": ""},
    ],
    narrator_metadata={"historian_1": {"model_family": "gpt-4"}},
)
check("Partial metadata -> independent (no shared lineage detected)",
...
g1 = reg_edge.record_evidence("edge_narrator", "physics",
...
for i in range(4):
    reg_edge.record_evidence("edge_narrator", "physics",
                             EvidenceType.CORROBORATION_OUTCOME, EvidenceAction.TADIL)
g2 = reg_edge.get_grade("edge_narrator", "physics")
check("WEAK -> ACCEPTABLE (5 TADIL total)", g2 == NarratorGrade.ACCEPTABLE,
      f"got {g2.value}")
...
check("ACCEPTABLE -> RELIABLE (10 TADIL total)", g3 == NarratorGrade.RELIABLE,
      f"got {g3.value}")
...
check("RELIABLE -> WEAK (3 JARH: Beta(11,4) mean=0.73 < 0.75)",
...
check("WEAK -> REJECTED (14 JARH total: Beta(11,15) mean=0.42)",
