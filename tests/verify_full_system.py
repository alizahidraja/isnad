...
check("NarratorGrade ordering: RELIABLE > ACCEPTABLE",
      NarratorGrade.RELIABLE > NarratorGrade.ACCEPTABLE)
check("NarratorGrade ordering: ACCEPTABLE > WEAK",
      NarratorGrade.ACCEPTABLE > NarratorGrade.WEAK)
check("NarratorGrade ordering: WEAK > REJECTED",
      NarratorGrade.WEAK > NarratorGrade.REJECTED)
check("NarratorGrade ordering: UNGRADED < WEAK",
      NarratorGrade.UNGRADED < NarratorGrade.WEAK)
check("NarratorGrade.is_at_least_acceptable: RELIABLE",
      NarratorGrade.RELIABLE.is_at_least_acceptable)
check("NarratorGrade.is_at_least_acceptable: WEAK=False",
      not NarratorGrade.WEAK.is_at_least_acceptable)
check("NarratorGrade.min selects lowest",
...
check("ChainGrade.min selects lowest",
      ChainGrade.min(ChainGrade.SAHIH, ChainGrade.DAIF) == ChainGrade.DAIF)

check("TransformType distinct values",
      len({TransformType.DESTRUCTIVE, TransformType.GENERATIVE, TransformType.PASS_THROUGH}) == 3)

check("NarratorType covers all taxonomy",
      len({NarratorType.SOURCE, NarratorType.SCRAPER, NarratorType.MODEL, NarratorType.HUMAN}) == 4)

check("AdalahGrade + DabtGrade distinct axes",
      AdalahGrade.HIGH != DabtGrade.HIGH)

check("Action covers all matrix outputs",
      len({Action.SERVE, Action.SERVE_WITH_CAVEAT, Action.REVIEW,
           Action.QUARANTINE, Action.REJECT_AND_QUARANTINE_NARRATOR}) == 5)

check("ContentVerdict covers all critic outputs",
      len({ContentVerdict.CONSISTENT, ContentVerdict.CONTRADICTION, ContentVerdict.UNVERIFIABLE}) == 3)

check("EvidenceType includes all evidence classes",
      len({EvidenceType.EVAL_HARNESS, EvidenceType.POST_HOC_AUDIT,
           EvidenceType.CORROBORATION_OUTCOME, EvidenceType.HUMAN_REVIEW,
           EvidenceType.VERSION_BUMP, EvidenceType.BOOTSTRAP_SEED}) == 6)

check("EvidenceAction jarh/tadil/neutral",
...
check("SQLAlchemy Base has all tables",
      all(t in Base.metadata.tables for t in
          ["rijal_claims", "chain_links", "narrator_registry",
           "narrator_evidence", "review_queue"]))

# Insert + read back
with get_session() as session:
    claim = RijalClaim(
        claim_id="test-claim-1", page_slug="physics/intro",
        claim_text="F = ma", normalized_text="f = m a",
        narrator_chain=[], chain_grade="hasan",
    )
    session.add(claim)
    session.flush()

    narrator = NarratorRegistry(
        narrator_id="model-x", domain_tag="physics",
    ...
    evidence = NarratorEvidence(
        narrator_id="model-x", domain_tag="physics",
        evidence_type="eval_harness", action="tadil",
    ...
    check("RijalClaim persisted + read back",
    ...
    check("NarratorRegistry persisted + read back",
    ...
    check("NarratorEvidence persisted + read back",
...
for i in range(4):
    reg_b.record_evidence("new", "physics", EvidenceType.CORROBORATION_OUTCOME,
                          EvidenceAction.TADIL, f"OK {i}")
...
reg_b2 = Registry()
for i in range(3):
    reg_b2.record_evidence("bad", "physics", EvidenceType.POST_HOC_AUDIT,
                           EvidenceAction.JARH, f"fail {i}")
...
check("Bayesian seed: prior mean ≈ 0.79", 0.78 < state.mean < 0.80,
      f"mean={state.mean:.3f}")
check("Bayesian seed: prior grade = ACCEPTABLE",
      state.to_grade() == NarratorGrade.ACCEPTABLE)
...
check("Domain-conditioned: same narrator, different domains",
...
check("Adalah + Dabt stored independently",
      n is not None and n.adalah_grade == AdalahGrade.HIGH
      and n.dabt_grade == DabtGrade.LOW)

# Quarantine
reg_axes.quarantine("src-A", "physics", "injection")
n2 = reg_axes.get("src-A", "physics")
check("Quarantine: grade=REJECTED, adalah=COMPROMISED, inactive",
      n2 is not None and n2.grade == NarratorGrade.REJECTED
...
check("get_metadata returns model_family",
      meta.get("model_family") == "gpt-4")
check("get_metadata returns upstream_source",
      meta.get("upstream_source") == "openai")
...
chain = Chain([
    ChainLinkSpec("source:openstax", 0, transform_type=TransformType.PASS_THROUGH,
                  domain="physics"),
    ChainLinkSpec("pdf-scraper@1.2", 1, transform_type=TransformType.DESTRUCTIVE,
                  domain="physics"),
    ChainLinkSpec("ingest_model", 2, transform_type=TransformType.GENERATIVE,
                  domain="physics", version="v1"),
])
check("Complete chain is complete", chain.is_complete)
check("Chain has 3 links", len(chain) == 3)
check("Narrator IDs ordered", chain.narrator_ids ==
      ["source:openstax", "pdf-scraper@1.2", "ingest_model"])

# Gap → munqati
chain_gap = Chain([
    ChainLinkSpec("a", 0), ChainLinkSpec("c", 2),
])
...
check("JSONB contains transform_type",
      jsonb[1]["transform_type"] == "destructive")

# Claim normalization
check("normalize_claim_text: lowercase + strip",
      normalize_claim_text("  F = MA  ") == "f = ma")
check("make_claim_id: deterministic SHA-256",
      make_claim_id("F = ma") == make_claim_id("f = ma"))
check("make_claim_id: different claims different IDs",
      make_claim_id("F = ma") != make_claim_id("p = mv"))

# Chain persistence
with get_session() as session:
    stored = store_claim(session, "F = ma", "physics/intro", chain,
                         chain_grade="hasan")
...
check("Destructive WEAK → permanent floor (no corroboration)",
      cg6 == ChainGrade.DAIF, f"got {cg6.value}")
...
check("Generative with corroboration can repair destructive damage",
      cg7 > ChainGrade.DAIF, f"got {cg7.value}")
...
check("Generative ACCEPTABLE cannot reach SAHIH",
      cg8 == ChainGrade.HASAN, f"got {cg8.value}")
...
check("No corroborating chains → base",
      pol.compute_corroborated_grade(ChainGrade.DAIF, [], []) == ChainGrade.DAIF)
check("MAWDU never upgraded",
      pol.compute_corroborated_grade(ChainGrade.MAWDU,
                                     [ChainGrade.SAHIH, ChainGrade.SAHIH],
                                     [1.0, 1.0]) == ChainGrade.MAWDU)
check("Only DAIF chains → gate fails",
      pol.compute_corroborated_grade(ChainGrade.DAIF,
                                     [ChainGrade.DAIF, ChainGrade.DAIF],
                                     [1.0, 1.0]) == ChainGrade.DAIF)
check("2 HASAN independent → DAIF upgraded to HASAN",
      pol.compute_corroborated_grade(ChainGrade.DAIF,
                                     [ChainGrade.HASAN, ChainGrade.HASAN],
                                     [1.0, 1.0]) == ChainGrade.HASAN)
check("HASAN cannot reach SAHIH via corroboration",
      pol.compute_corroborated_grade(ChainGrade.HASAN,
                                     [ChainGrade.SAHIH, ChainGrade.SAHIH, ChainGrade.SAHIH],
                                     [1.0, 1.0, 1.0]) == ChainGrade.HASAN)
check("Correlated chains discounted (effective < 2 → no upgrade)",
      pol.compute_corroborated_grade(ChainGrade.DAIF,
                                     [ChainGrade.HASAN, ChainGrade.HASAN],
                                     [0.5, 0.5]) == ChainGrade.DAIF)
...
check("evaluate_corroboration: 2 independent HASAN → upgrade to HASAN",
      r_int == ChainGrade.HASAN, f"got {r_int.value}")
...
check("evaluate_corroboration: all gpt-4 family → no upgrade (madar)",
      r_corr == ChainGrade.DAIF, f"got {r_corr.value}")
...
r_eng1 = engine.evaluate(
    ...
    all_chains=[
        {"claim_text": "energy is conserved", "chain_grade": "hasan",
         "narrator_ids": ["source:C", "parser:v2"], "source": ""},
        {"claim_text": "energy is conserved", "chain_grade": "hasan",
         "narrator_ids": ["source:D", "parser:v3"], "source": ""},
    ],
    narrator_metadata={},
)
check("Engine: DAIF + 2 independent HASAN → upgrade fires",
      r_eng1.upgraded, f"reason: {r_eng1.reason}")
check("Engine: upgraded to HASAN",
      r_eng1.upgraded_grade == ChainGrade.HASAN,
      f"got {r_eng1.upgraded_grade.value}")
check("Engine: 2 corroborating, 2 independent",
...
r_eng2 = engine.evaluate(
    ...
    all_chains=[
        {"claim_text": "momentum is mass times velocity", "chain_grade": "hasan",
         "narrator_ids": ["model_gpt4o"], "source": ""},
        {"claim_text": "momentum is mass times velocity", "chain_grade": "hasan",
         "narrator_ids": ["model_gpt4_turbo"], "source": ""},
        {"claim_text": "momentum is mass times velocity", "chain_grade": "hasan",
         "narrator_ids": ["model_claude"], "source": ""},
    ],
    narrator_metadata={
...
check("Engine: madar blocks upgrade (gpt-4 family)", not r_eng2.upgraded,
...
r_eng3 = engine.evaluate(
    ...
    all_chains=[{"claim_text": "fake news", "chain_grade": "sahih",
...
check("Engine: effective_weight computed",
      r_eng1.effective_weight > 1.0, f"weight={r_eng1.effective_weight:.2f}")
...
check("EmbeddingCritic exact match → CONSISTENT",
      r_c2 == ContentVerdict.CONSISTENT, f"got {r_c2.value}")
...
check("EmbeddingCritic negation → CONTRADICTION",
      r_c3 == ContentVerdict.CONTRADICTION, f"got {r_c3.value}")

r_c4 = ec.evaluate("test", "test", [], "physics")
check("EmbeddingCritic empty corpus → UNVERIFIABLE",
      r_c4 == ContentVerdict.UNVERIFIABLE)
...
check("EmbeddingCritic negation → CONTRADICTION",
      r_c5 == ContentVerdict.CONTRADICTION, f"got {r_c5.value}")

# ── HybridCritic fallback ──
hc = HybridCritic()
r_h1 = hc.evaluate("test", "test", [], "physics")
check("HybridCritic empty corpus → UNVERIFIABLE",
      r_h1 == ContentVerdict.UNVERIFIABLE)

# ── TF-IDF Index ──
idx = TFIDFIndex(["force equals mass times acceleration",
                   "momentum is mass times velocity"])
# Same text → sim = 1.0
...
check("LocalNLICritic: graceful when no model",
...
check("describe_action SAHIH+CONTRADICTION mentions ilal",
      "ilal" in desc.lower() or "ʿilal" in desc)
...
data = r.json()
check("Metrics has corroboration_fires_total",
      "corroboration_fires_total" in data)
check("Metrics has bayesian_grade_changes_total",
      "bayesian_grade_changes_total" in data)
check("Metrics has claims_submitted_total",
      "claims_submitted_total" in data)
...
r = client.post("/v1/narrators", json={"narrator_id": "x"},
...
check("Claim has corroborating_claims count",
      "corroborating_claims" in claim_data)

# Get claim
r = client.get(f"/v1/claims/{cid}")
check("GET /v1/claims/{id} → 200", r.status_code == 200)
check("Claim text matches", r.json()["claim_text"] == "F = ma")
check("Corroborating claims count present",
      "corroborating_claims" in r.json())
...
list_data = r.json()
check("Claims list has total", list_data["total"] >= 1,
      f"total={list_data['total']}")
...
check("Pagination: limit=1 returns ≤1 claim",
      len(r.json()["claims"]) <= 1)

# Register narrator
r = client.post("/v1/narrators", json={
    "narrator_id": "model:api_test", "grade": "acceptable",
}, headers={"X-API-Key": "isnad-admin"})
...
check("Narrator grade = acceptable",
      r.json()["grade"] == "acceptable")
...
check("Same narrator, different domains: physics=reliable",
      r1.json()["grade"] == "reliable")
check("Same narrator, different domains: history=weak",
      r2.json()["grade"] == "weak")

# Submit evidence
r = client.post("/v1/evidence", json={
    "narrator_id": "model:api_test", "action": "jarh",
...
check("Two claims stored successfully",
...
check("_build_policy() default → BayesianTransitionPolicy",
...
check("corroboration_fires_total is int",
      isinstance(_metrics_counters["corroboration_fires_total"], int))
check("bayesian_grade_changes_total is int",
      isinstance(_metrics_counters["bayesian_grade_changes_total"], int))
check("claims_submitted_total is int",
      isinstance(_metrics_counters["claims_submitted_total"], int))
