...
reg = Registry()
check(
    "Registry() default is BayesianTransitionPolicy",
...
check(
    "ThresholdTransitionPolicy remains as named fallback",
    isinstance(reg_t.transition_policy, ThresholdTransitionPolicy),
)

# Bayesian: 1 positive → WEAK (Beta(2,1), mean=0.67)
g1 = reg.record_evidence(
    "bayes_test", "physics", EvidenceType.EVAL_HARNESS, EvidenceAction.TADIL, "Pass 1"
)
check("1 TADIL → WEAK (not RELIABLE)", g1 == NarratorGrade.WEAK, f"got {g1.value}")

# Bayesian: 3 adverse → REJECTED (Beta(1,4), mean=0.20)
reg2 = Registry()
for i in range(3):
    reg2.record_evidence(
        "bad_model", "physics", EvidenceType.POST_HOC_AUDIT, EvidenceAction.JARH, f"Fail {i}"
    )
g3 = reg2.get_grade("bad_model", "physics")
check(
    "3 JARH → REJECTED (Bayesian dominates fast)", g3 == NarratorGrade.REJECTED, f"got {g3.value}"
)

# Bayesian: sustained → ACCEPTABLE (5 TADIL + 1 JARH)
reg3 = Registry()
for i in range(5):
    reg3.record_evidence(
        "good_model", "physics", EvidenceType.CORROBORATION_OUTCOME, EvidenceAction.TADIL, f"OK {i}"
    )
reg3.record_evidence(
    "good_model", "physics", EvidenceType.POST_HOC_AUDIT, EvidenceAction.JARH, "minor"
)
g4 = reg3.get_grade("good_model", "physics")
check("5 TADIL + 1 JARH → ACCEPTABLE", g4 == NarratorGrade.ACCEPTABLE, f"got {g4.value}")

# Bayesian: many positives → RELIABLE
reg4 = Registry()
for i in range(10):
    reg4.record_evidence(
        "excellent", "physics", EvidenceType.CORROBORATION_OUTCOME, EvidenceAction.TADIL, f"OK {i}"
    )
g5 = reg4.get_grade("excellent", "physics")
check("10 TADIL → RELIABLE", g5 == NarratorGrade.RELIABLE, f"got {g5.value}")

# Version bump still resets
reg4.bump_version("excellent", "physics", "v2")
g6 = reg4.get_grade("excellent", "physics")
check("Version bump → UNGRADED", g6 == NarratorGrade.UNGRADED, f"got {g6.value}")
...
check(
    "Shared model family → no upgrade (madār detected)",
    not result2.upgraded,
    ...
    all_chains=[
        {
            "claim_text": "F equals ma",
            "chain_grade": "hasan",
            "narrator_ids": ["source:C"],
            "source": "",
        },
        {
            "claim_text": "F equals ma",
            "chain_grade": "hasan",
            "narrator_ids": ["source:D"],
            "source": "",
        },
    ],
    narrator_metadata={
...
result4 = engine.evaluate(
    ...
    base_narrators=["bad"],
    all_chains=[
        {"claim_text": "poison", "chain_grade": "sahih", "narrator_ids": ["good"], "source": ""},
    ],
    narrator_metadata={},
)
...
check(
    "Different claim text → no corroboration",
    not result5.upgraded,
...
with get_session() as session:
    ...
    check(
        "Grade survives across sessions (DB persistence)",
...
check(
    ...
    cg_incomplete == ChainGrade.DAIF,
    f"got {cg_incomplete.value}",
)
...
check("HybridCritic empty corpus → UNVERIFIABLE", result_empty == ContentVerdict.UNVERIFIABLE)
...
check(
    ...
    result_exact == ContentVerdict.UNVERIFIABLE,
...
check(
    "EmbeddingCritic negation contradiction → CONTRADICTION",
